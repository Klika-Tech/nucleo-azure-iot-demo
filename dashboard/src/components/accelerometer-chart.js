...
import { lineOptimized } from '../services/d3utils';
...
function initChart(el) {
    ...
    function updateData(brushDomain = null) {
        contextData = prepareData(dataService.get().sensorData);
    ...
    function updateFocusChart() {
        const minimal = d3.min(contextData.map(d => Math.min(d.accelerometer.x, d.accelerometer.y, d.accelerometer.z)));
        const maximal = d3.max(contextData.map(d => Math.max(d.accelerometer.x, d.accelerometer.y, d.accelerometer.z)));
...
const AccelerometerChart = () => (
  <div className="temperature-chart-container">
    <div className="magnetometer-chart" ref={initChart} />
  </div>
);
