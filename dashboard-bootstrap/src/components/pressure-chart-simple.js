...
const PressureChart = React.createClass({
	...
	initChart: function(el) {
		...
		d3.select(window).on('resize.pressure', function() {
			setDimensions()
			that.updateChart()
		})
		...
		this.updateChart = function() {
			...
			y.domain([
					Math.floor((d3.min(focusData.map(function(d) { return d.pressure })) - .3) * 30) / 30,
					Math.ceil((d3.max(focusData.map(function(d) { return d.pressure })) + .3) * 30) / 30
				])
